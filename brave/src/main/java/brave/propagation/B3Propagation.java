...
public abstract class B3Propagation<K> implements Propagation<K> {
  ...
  public enum Format implements InjectorFunction {
    MULTI() {
      ...
      @Override public <R> void inject(Setter<R, String> setter, TraceContext context, R request) {
        ...
        if (parentId != null){
          setter.put(request, PARENT_SPAN_ID, parentId);
        }
        if (context.debug()) {
  ...
  public static final class FactoryBuilder {
    // InjectorFactory工厂类
    InjectorFactory.Builder injectorFactoryBuilder = InjectorFactory
        .newBuilder(Format.MULTI)
        .clientInjectorFunctions(Format.MULTI) // 客户端
        .producerInjectorFunctions(Format.SINGLE_NO_PARENT) // 生产者端
        .consumerInjectorFunctions(Format.SINGLE_NO_PARENT); // 消费者端
  ...
  static final String SAMPLED_MALFORMED = "Invalid input: expected 0 or 1 for " + SAMPLED + ", but found '{0}'";
  ...
  static final class Factory extends Propagation.Factory implements Propagation<String> {
    ...
    @Override public <K1> Propagation<K1> create(KeyFactory<K1> keyFactory) {
      // Propagation<String> delegate,KeyFactory
  ...
  static final class B3Extractor<R> implements Extractor<R> {
    ...
    @Override public TraceContextOrSamplingFlags extract(R request) {
      ...
      if (traceIdString == null) {
        ...
        if (sampledV != null) {
          return sampledV ? TraceContextOrSamplingFlags.SAMPLED : TraceContextOrSamplingFlags.NOT_SAMPLED;
        }
      }
      // Try to parse the trace IDs into the context
      TraceContext.Builder result = TraceContext.newBuilder();
      if (result.parseTraceId(traceIdString, TRACE_ID) && result.parseSpanId(getter, request, SPAN_ID)
